...
async function executeOcrVisionStep({
    ...
    const ocrSystemInstruction = [
        ...
        "CONJOINED / CONNECTED / TOUCHING SPEECH BUBBLES RULE (BÓNG THOẠI DÍNH LIỀN NHAU / LIÊN HOÀN):",
        "- When 2 or more speech bubbles are connected, touching, conjoined, chained, or overlapping in a multi-lobe bubble cluster (e.g. an upper-right bubble lobe containing '私にもいつか' attached to a lower-left bubble lobe containing '糸出るのかな'):",
        "- You MUST split and output EACH bubble lobe as a SEPARATE individual block! NEVER merge connected/chained bubble lobes into a single block.",
        "- For each bubble lobe, output its own distinct text and its own distinct center anchor [x, y] located at the center of THAT specific lobe.",
        "- Order connected lobes in natural reading flow (Upper-Right lobe first, then Lower-Left lobe next).",
        "JAPANESE VERTICAL TEXT & MULTI-COLUMN SPECIFICATION (縦書き):",
        "1. SINGLE BUBBLE WITH MULTI-COLUMNS: Within a SINGLE standalone speech bubble chamber containing 2 to 5 parallel vertical columns, output them as ONE unified block. (Do not confuse this with connected/conjoined bubble lobes, which must be split into separate blocks per lobe).",
        ...
        try {
            ...
            if (pipelineMode === 'two-step') {
                ...
                const systemInstruction = [
                    ...
                    "CONJOINED / CONNECTED / TOUCHING SPEECH BUBBLES RULE (BÓNG THOẠI DÍNH LIỀN NHAU / LIÊN HOÀN):",
                    "- When 2 or more speech bubbles are connected, touching, conjoined, chained, or overlapping in a multi-lobe bubble cluster (e.g. upper-right bubble lobe connected to lower-left bubble lobe):",
                    "- You MUST split and output EACH bubble lobe as a SEPARATE individual block! NEVER merge connected/chained bubble lobes into a single block.",
                    "- For each bubble lobe, output its own distinct text and its own distinct center anchor [x, y] located at the center of THAT specific lobe.",
                    "JAPANESE VERTICAL TEXT & MULTI-COLUMN SPECIFICATION (縦書き):",
                    "1. SINGLE BUBBLE WITH MULTI-COLUMNS: Within a SINGLE standalone speech bubble chamber containing 2 to 5 parallel vertical columns, output them as ONE unified block. (Do not confuse this with connected/conjoined bubble lobes, which must be split into separate blocks per lobe).",
