...
async function executeOcrVisionStep({
    ...
    const ocrSystemInstruction = [
        ...
        "- For conjoined / connected / touching multi-lobe bubble clusters (e.g. upper-right bubble lobe attached to lower-left bubble lobe): You MUST split and output EACH bubble lobe as a SEPARATE individual block with its own center anchor.",
        ...
        "1. SINGLE BUBBLE WITH MULTI-COLUMNS: Within a SINGLE standalone speech bubble chamber containing 2 to 5 parallel vertical columns, output them as ONE unified block.",
        ...
        try {
            ...
            if (pipelineMode === 'two-step') {
                ...
                const systemInstruction = [
                    ...
                    "- Every individual speech bubble, narration box, or floating text cluster MUST be output as its own separate block with its own distinct center anchor [x, y].",
                    "- DO NOT combine or merge two separate speech bubbles into one block, EVEN IF they are close to each other, in the same panel, spoken by the same character, or part of the same sentence.",
                    "- For conjoined / connected / touching multi-lobe bubble clusters: You MUST split and output EACH bubble lobe as a SEPARATE individual block with its own center anchor.",
                    "JAPANESE VERTICAL TEXT & MULTI-COLUMN SPECIFICATION (縦書き):",
                    "1. SINGLE BUBBLE WITH MULTI-COLUMNS: Within a SINGLE standalone speech bubble chamber containing 2 to 5 parallel vertical columns, output them as ONE unified block.",
