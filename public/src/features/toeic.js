...
export function quickOpenToeicAnalysis() {
    uiSetRightTab('toeic');
}

// 2. Cập nhật giao diện tab TOEIC
export function updateToeicTabUI() {
...
export function resetToeicAnalysisUI() {
...
export function displayToeicAnalysis(analysis) {
    if (!analysis) return;
...
export function checkToeicAnswer(selectedLetter, correctLetter, explanation) {
...
export async function analyzeBlockForToeic() {
...
export function updateToeicNotebookUI() {
...
export function closeSrsReviewModal() {
    const modal = document.getElementById('srs-review-modal');
    if (modal) modal.classList.add('hidden');
    updateToeicNotebookUI();
}

export function renderSrsCurrentCard() {
...
export function speakSrsCurrentWord() {
    if (srsCurrentIndex < srsReviewQueue.length) {
        const item = srsReviewQueue[srsCurrentIndex];
        if (item && item.word) {
            speakText(item.word);
...
export async function submitSrsReview(quality) {
...
export async function deleteSavedToeicWord(index) {
...
export function exportToeicWordsToAnki() {
    const savedWords = globalState.toeicSavedWords || [];
    if (savedWords.length === 0) {
...
export function setToeicMode(mode) {
...
export function showToeicRecallHint() {
    if (globalState.activePageIndex === -1 || globalState.selectedBlockId === null) return;
...
export function checkToeicRecall() {
    if (globalState.activePageIndex === -1 || globalState.selectedBlockId === null) return;
...
export function getSimpleWordDiff(userText, correctText) {
...
export function speakCorrectRecallSentence() {
    if (globalState.activePageIndex === -1 || globalState.selectedBlockId === null) return;
    const page = globalState.pages[globalState.activePageIndex];
    const block = page.blocks.find(b => b.id === globalState.selectedBlockId);
    if (block && block.original) {
...
export function selectToeicQuestion(index) {
...
export function renderActiveToeicQuestion(pqs, index) {
    const pq = pqs[index];
    if (!pq) return;
