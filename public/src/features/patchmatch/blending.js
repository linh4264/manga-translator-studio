...
/**
 * Computes exact Euclidean/Manhattan distance transform of masked pixels to the nearest unmasked pixel
 * @param {Uint8Array} mask - Binary mask (1 = masked, 0 = unmasked)
 * @param {number} width - Image width
 * @param {number} height - Image height
 * @returns {Float32Array} - Distance array in pixels
 */
export function computeDistanceToBoundary(mask, width, height) {
    const total = width * height;
    const dist = new Float32Array(total);
    const INF = 999999.0;

    // Initialize: unmasked pixels have distance 0, masked have INF
...
/**
 * Blends reconstructed image seamlessly with original image at mask boundaries
 * @param {Uint8Array} originalRgba - Original RGBA image buffer
 * @param {Uint8Array} reconstructedRgba - Reconstructed RGBA image buffer
 * @param {Uint8Array} mask - Binary mask (1 = inpaint, 0 = keep)
 * @param {number} width - Image width
 * @param {number} height - Image height
 * @param {number} blendRadius - Feather radius in pixels (default: 2.5)
 * @returns {Uint8Array} - Seamless blended RGBA image buffer
 */
export function applySeamlessBoundaryBlending(
    ...
    for (let i = 0; i < total; i++) {
        if (mask[i] === 1) {
            ...
            if (d >= bRad) {
                // Fully inside reconstructed region
                ...
                // Smooth Cosine S-Curve blending: 0 at boundary -> 1 at blendRadius
                const t = d / bRad;
