...
export async function createMangaPSD(page, originalImgEl, eraserCanvas) {
    ...
    try {
        ...
        if (agPsd && (agPsd.writePsd || agPsd.writePsdUint8Array)) {
            ...
            const textChildren = (page.blocks || []).map((block, idx) => {
                ...
                // Render styled text block onto local canvas
    ...
    // Standalone Zero-Dependency Pure JS PSD Binary Generator (100% Offline)
    return writeStandalonePSD(width, height, cleanCanvas, rawCanvas);
}

/**
 * Lightweight Zero-Dependency Standalone Pure-JS PSD Generator
 */
function writeStandalonePSD(width, height, cleanCanvas, rawCanvas) {
    const cleanCtx = cleanCanvas.getContext('2d');
    const rawCtx = rawCanvas.getContext('2d');
    ...
    // 4 channels: R, G, B, A (each pixelCount bytes)
    const channelSize = pixelCount;
    const layerDataSize = channelSize * 4;
    ...
    // We create 2 layers: Clean Background and Raw Background
    const layerCount = 2;
    const layerRecords = [];

    // Extract Planar Channels (R, G, B, A)
    function extractPlanarChannels(imgData) {
    ...
    const pushLBytes = (arr) => {
        layerInfoParts.push(new Uint8Array(arr));
    };

    pushL16(layerCount); // Layer count

    // Layer 1 Record: Clean Background
    pushL32(0); pushL32(0); pushL32(height); pushL32(width); // Top, Left, Bottom, Right
    pushL16(4); // 4 Channels
    // Channel headers (id, length)
    pushL16(0); pushL32(2 + pixelCount);  // Channel 0 (R)
    pushL16(1); pushL32(2 + pixelCount);  // Channel 1 (G)
    pushL16(2); pushL32(2 + pixelCount);  // Channel 2 (B)
    pushL16(-1); pushL32(2 + pixelCount); // Channel -1 (A)
    pushLStr('8BIM'); pushLStr('norm');   // Blend mode
    pushLBytes([255, 0, 0, 0]);           // Opacity 255, Clipping 0, Flags 0, Filler
    // Extra data (Layer Name: "Clean Background")
    const name1 = "Clean Background";
    const name1Len = name1.length;
    const pad1 = (4 - ((1 + name1Len) % 4)) % 4;
    pushL32(4 + 4 + (1 + name1Len + pad1)); // Extra data length
    pushL32(0); // Mask data length 0
    pushL32(0); // Blending ranges length 0
    pushLBytes([name1Len]);
    pushLStr(name1);
    ...
    // Channel Image Data for Layer 1 (Clean) - Uncompressed (Compression = 0)
    ...
    // Channel Image Data for Layer 2 (Raw) - Uncompressed
    ...
    // Write Layer and Mask Section Header
    writeUint32(layerInfoLength + 4); // Layer and Mask info total length
    writeUint32(layerInfoLength);     // Layer info length
    parts.push(...layerInfoParts);

    // 5. Global Image Data (Merged composite for quick preview)
    writeUint16(0); // Compression: 0 = Raw uncompressed
